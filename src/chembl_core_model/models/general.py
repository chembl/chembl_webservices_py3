...
class Version(six.with_metaclass(ChemblModelMetaClass, ChemblCoreAbstractModel)):
    ...
    name = ChemblCharField(primary_key=True, max_length=20, help_text='Name of release version')
    creation_date = ChemblDateField(blank=True, null=True, help_text='Date database created')
    comments = ChemblCharField(max_length=2000, blank=True, null=True, help_text='Description of release version')
...
class ChemblIdLookup(six.with_metaclass(ChemblModelMetaClass, ChemblCoreAbstractModel)):
    ...
    chembl_id = ChemblCharField(primary_key=True, max_length=20, help_text='ChEMBL identifier')
    entity_type = ChemblCharField(max_length=50, choices=ENTITY_TYPE_CHOICES, help_text='Type of entity (e.g., COMPOUND, ASSAY, TARGET)')
    ...
    status = ChemblCharField(max_length=10, default='ACTIVE', choices=STATUS_CHOICES, help_text='Indicates whether the status of the entity within the database - ACTIVE, INACTIVE (downgraded), OBS (obsolete/removed).')
